...
Languages provided: `english`, `german`, `danish`, `french`, and **`wehrmacht`** (no
default — the scoring language must be given with `-l` for the n-gram models).
`wehrmacht` is telegraphic military German (X as word separator, Q for *ch*, spelled-out
numbers), generated by `eval/build_telegraphic_ngrams.py` from the published Appendix-C
statistics; it is a *register*, so use it for real WWII traffic and `german` for prose
(measured +20.9pp on real messages, −10.2pp on prose — `eval/MODERN_BREAKING_NOTES.md` §6).

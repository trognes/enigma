...
*writing style*, not a separate language, so use it for real WWII traffic and `german`
for prose (measured +20.9pp on real messages, −10.2pp on prose —
...
> scoring model, and the only one that does not depend on the writing style; see the `-f` entry below), `-S m4f10` staging (mono pre-pass then fused), `-J` (dynamic

...
is telegraphic military German (X fencing abbreviations, numerals, names and
clause breaks; Q for *ch*; spelled-out numbers), generated by
`eval/build_telegraphic_ngrams.py` from the published
...
messages, −10.2pp on prose — `eval/MODERN_BREAKING_NOTES.md` §6). The
eleventh, **`hgnord`**, is `wehrmacht` plus the counted n-grams of the 48
clean authentic HG Nord 1941 decrypts in `eval/` (`eval/build_hgnord_ngrams.py`,
w = 1000): an in-network prior for that network's traffic, measured 55%
against 39% of 80-letter messages broken held out by message. **It has seen
every corpus message, so no eval here may use it** — see "Status & remaining
work".

> **X is a DISAMBIGUATION MARKER, NOT A SPACE — this entry said "X as word
> separator" for a long time and it is wrong.** Measured over the 75 authentic
> decrypts: X is 6.61% of letters, but the 543 X-delimited chunks average
> **12.7 letters** (median 9, p90 28, longest **121**) against a ~6-letter
> German word, and **32% of them exceed 12 letters** — i.e. a chunk holds about
> two words, and a third of them certainly hold several. What X fences is what
> a reader could otherwise mis-segment: abbreviations (`DIV`, `INF`, `ABT`,
> `LKW`, `KM`, `PANZ`, `GEZ`), spelled-out numerals (`SIEBEN`, `ZWO`, `NEUN`,
> `EINS`) and proper names (`SCHUSTER`, `HENNING`, `ZANDERS`, `PORCHOW`), plus
> clause and sentence breaks. Running prose is simply concatenated —
> `UHRMITANFAENGENAMUNTERKUNFTSRAUM` is five words and `KANNNIQTEINFLIESZEN`
> is three, both unbroken.
>
> Nothing in the code assumed otherwise, but the loose wording would mislead
> anyone sizing a crib, a doubling null, or a segmentation prior on "an X every
> six letters". It is also why **`--self-crib-tandem`** exists: a doubled name
> is *usually* X-fenced, which is what `--doubling-report` and the default
> self-crib look for, and the corpus's very first message carries
> `SIEGFRIEDSIEGFRIEDTONI` with no separator at all. N-gram files
...
- `-S k` / `--score k…` **mono + IC fused, as a PRE-PASS model**
  ...
  - **The exponent is nominally WRONG, and correcting it measured NULL — do
    not "fix" it again.** What `λ` has to track is `sd(mono)/sd(IC)`, measured
    7.4 / 8.8 / 11.0 / 14.2 at L = 40/60/100/167, i.e. about `1.1·√L`. A
    *linear* rule crosses that curve exactly once — at **L = 121** — so
    `0.1·L` undershoots below and overshoots above, giving `r` (IC's weight in
    units of each statistic's own spread) = 0.54 / 0.68 / 0.91 / 1.18 instead
    of a constant. `λ = 1.1·√L`, which holds `r` = 0.94 / 0.97 / 1.00 / 1.00,
    was built and A/B'd against it — paired, both arms in one binary via
    `ENIGMA_MONOIC_BLEND`, n = 2000 at three lengths: **+0.84 / −0.75 / −0.93pp
    at L = 167/100/60, pooled −0.40pp [−1.09, +0.28]**. Every interval spans
    zero and the sign is mildly *against* the tidier rule, including at L = 60
    where `r` moved furthest. The surface is a broad plateau — every `r` in
    0.25…2.0 beats both pure statistics at L ≥ 60 — so the exponent does not
    matter empirically, and `0.1·L` additionally tracks the probe's *peak* `r`,
    which itself rises with length (0.5 / 1.0 / 1.0 / 1.0–2.0). See
...
- **Search — resolved, compute-bound with no selectable shortcut.** At short
  ...
  **the weight curve does not peak**: at w = 1000, where the stock table
  ...
  would contaminate every later measurement. **It ships as `-l hgnord`**
  instead: the same mix at w = 1000, counted from all 48 clean decrypts by
  `eval/build_hgnord_ngrams.py` (which prints the message set), with its
  own `lang_coeffs` row copying wehrmacht's `0.25·L`. **No eval harness may
  use `hgnord` on the corpus messages** — the only honest use on real
  traffic is the messages that were never read. `ENHANCEMENTS.md` 2b.

...
/* X-delimiter structure term (EXPERIMENTAL, off by default; see the block
   above xstruct_init in scoring.cc). Read from $ENIGMA_XSTRUCT /
   $ENIGMA_XGAPS at startup; mu = 0 leaves the scorer byte-identical. */
void xstruct_init();
double xstruct_mu();

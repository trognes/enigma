...
# 21/22. Restart-gain sweep: recovery vs -R at short lengths (-J -S i4q10).
# One line per length; x = restart budget (categorical, since R=0 has no log
# point). Each length drawn only where it has data (the high-R tail is run for
# the hardest lengths only). Recovery is restart-limited at the short end.
RSW_RS = [0, 1, 2, 5, 10, 20, 40, 80, 160, 320, 640, 1280,
          2560, 5120, 10240, 20480, 40960, 81920]
RSW_LENS = [40, 45, 50, 55, 60, 65, 70]


def restart_gain_plot(lang, subtitle, fname):
    rsw = defaultdict(list)
    for r in rows:
        cl = r["config_label"]
        if cl.startswith("rsw.J.R") and r["language"] == lang:
            try:
    ...
    have = [R for R in RSW_RS if any(rsw[(R, L)] for L in RSW_LENS)]
    if not have:
        return
    xpos = {R: i for i, R in enumerate(have)}
    ramp = [plt.cm.Blues(v) for v in [0.30, 0.40, 0.50, 0.60, 0.72, 0.84, 1.0]]
    fig, ax = plt.subplots(figsize=(9.5, 5.5))
    for L, color in zip(RSW_LENS, ramp):
        pts = [(xpos[R], sum(rsw[(R, L)]) / len(rsw[(R, L)])) for R in have if rsw[(R, L)]]
        if not pts:
            continue
        lx, ly = zip(*pts)
    ...
    ax.set_xticklabels([str(R) for R in have], rotation=45, ha="right", fontsize=8)
    ax.axhline(95, color="#888888", lw=1, ls=":")
    ax.set_xlabel("restart budget  -R   (compute ~ linear in R; ~2.3k score_iter/restart)")
    ax.set_ylabel("mean % letters correct")
    ax.set_title(f"Restart gain at short lengths ({lang}, -J -S i4q10, quad, 10 plugs, 2 seeds)\n"

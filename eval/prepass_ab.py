...
STILL OPEN: whether the flip is caused by the TARGET MODEL or by LENGTH.  A
single run at L=60 leaned the other way (+1.77pp for mono, CI spans 0), so a
length crossover fits the data just as well as a target-model effect.
Separating them needs this A/B across several lengths under both `-a` and `-f`.
...
def main():
    ...
    p = (2 * sum(comb(nd, k) for k in range(0, min(b, c) + 1)) / 2 ** nd
         if nd else 1.0)
    print(f"  McNemar on exact: m4-only {b}, i4-only {c}, discordant {nd}, "
          f"p={min(p, 1.0):.3f}")
    d = [x - y for x, y in zip(res[ARMS[0]], res[ARMS[1]])]
    se = statistics.stdev(d) / len(d) ** 0.5
    print(f"  paired diff (m4-i4): {statistics.mean(d):+6.2f} pp  "
          f"95% CI [{statistics.mean(d) - 1.96 * se:+.2f}, "
          f"{statistics.mean(d) + 1.96 * se:+.2f}]   (negative = i4f10 ahead)")
